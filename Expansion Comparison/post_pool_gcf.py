...
for nu in lst:
    df = pd.read_csv('gcf_check%d.csv' % nu, index_col=0)

    # Get the event data frame
    temp_dict = df.iloc[0, 4] # column: network addition(s), total cost, unserved energy, operation cost, all the other data
...
# %% print out to df
event_dff.to_csv('events_gcf_check.csv')
usenergy_dff.to_csv('usenergy_gcf_check.csv')
cost_dff.to_csv('cost_gcf_check.csv')
opcost_dff.to_csv('opcost_gcf_check.csv')
...
# %% The new code for gcf after pool (the pool results do not have the location of the events)
lst = [11]
cost_frame = []
usenergy_frame = []
opcost_frame = []
...
for nu in lst:
    ...
    event_df_lst = []
    for i in temp_list:
        event_df_lst.append([i[0], i[1],i[2]])
    event_df = pd.DataFrame(event_df_lst)
    print(len(event_df_lst))
    number_of_events += len(event_df_lst)
    event_frame.append(event_df)
...
print('number of GCF contingency scenarios is', number_of_events, 'in total')

event_dff.to_csv('events_gcf.csv')
